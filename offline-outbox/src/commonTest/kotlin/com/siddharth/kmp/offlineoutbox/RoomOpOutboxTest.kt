...
private class FakeOpOutboxDao : OpOutboxDao {
    ...
    override suspend fun pending(): List<OpOutboxEntity> = rows.value.filter { it.status == OpStatus.PENDING.name }.sortedBy { it.seq }

    override fun observeLive(): Flow<List<OpOutboxEntity>> =
        rows.map { list -> list.filter { it.status != OpStatus.DEAD.name }.sortedBy { it.seq } }

    override suspend fun dead(): List<OpOutboxEntity> = rows.value.filter { it.status == OpStatus.DEAD.name }.sortedBy { it.seq }
...
class RoomOpOutboxTest {
    @Test
    fun replay_drains_oldest_first_and_deletes_on_success() =
        runTest {
    ...
    @Test
    fun replay_stops_at_first_transient_failure_preserving_order() =
        runTest {
            val outbox = RoomOpOutbox(FakeOpOutboxDao())
            outbox.enqueue("op", "A")
            outbox.enqueue("op", "B")
    ...
    @Test
    fun replay_dead_letters_permanent_failure_and_continues() =
        runTest {
            val outbox = RoomOpOutbox(FakeOpOutboxDao())
            outbox.enqueue("op", "A")
            outbox.enqueue("op", "B")
    ...
    @Test
    fun replay_dead_letters_after_max_attempts_exhausted() =
        runTest {
            val outbox = RoomOpOutbox(FakeOpOutboxDao())
            outbox.enqueue("op", "A")
    ...
    @Test
    fun requeue_revives_a_dead_letter() =
        runTest {
            val outbox = RoomOpOutbox(FakeOpOutboxDao())
