...
class PeachGatewayTest {
    private fun prepared(params: Map<String, String>) =
        PreparedPayment(
            gatewayId = GatewayId("peach"),
            orderId = "order_1",
    ...
    fun `pay charges the saved instrument via the shared vault`() =
        runTest {
            val gateway = PeachGateway(FakeVaultBackend())
    ...
    fun `meta advertises the vault-pattern capability honestly`() {
        ...
        assertEquals("Peach Payments", gateway.meta.displayName)
        assertEquals(GatewayStatus.MOCK_MODE, gateway.meta.status)
        assertEquals(GatewayId("peach"), gateway.id)
    }
