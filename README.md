...
apps, never designed as a "platform" up front. Shared modules range from a typed `Result` primitive to a
...
`ai-testing`, `settings` and `app-shell` are Doori-only so far, the payment-gateway leaves serve
...
![Modules](https://img.shields.io/badge/modules-43-0EA5E9)
...
- [provider:\* (payment-gateway leaves)](#provider--payment-gateway-leaves)
...
<!-- module-counts:begin -->
> **At a glance:** **43 modules**, comprising **23 core/leaf modules**
> and **20 payment-gateway provider modules**. Generated from `settings.gradle.kts`.
<!-- module-counts:end -->
...
monorepo has since grown to include `llm-chat` (cloud LLM chat), the `payments-api` +
payment-gateway family, `offline-outbox` (the first Room module here), `bots-policy`
...
- 🧩 **Shared Gradle modules, one dependency graph.** Core/leaf modules (9 original + `llm-chat` /
  `payments-api` / `offline-outbox` / `bots-policy` + `device-integrity` / `settings` / `app-shell` /
  `store` + `auth` / `netlog` / `charts` + `ai-testing` + `biometric` / `secure-store`) and
...
- 💳 **`payments-api` and its providers is a real gateway-abstraction exercise, not a toy.** One
...
- ✅ **CI runs the full multiplatform matrix on every push**: `assemble check` (aggregating
  ...
  the declared modules, plus a dedicated no-AI-attribution check workflow.
...
| [**payments-api**](#payments-api) | `com.siddharth.kmp:payments-api` | `PaymentGateway`/`PaymentBackend` contract, `Money`, `PaymentResult`, redaction, `StubGateway` | Android · JVM · iOS | the `provider:*` leaves |
| [**provider:\***](#provider--payment-gateway-leaves) | `com.siddharth.kmp:provider-<name>` | Adapters implementing `payments-api`'s contract per gateway; mostly Android-only, `wallet` is KMP and `applepay` is iOS-only | mostly Android; `wallet` Android · iOS; `applepay` iOS only | reference integrations |
...
Generated from `settings.gradle.kts` and each module's `project(":x")` declarations by
`scripts/module-graph.mjs`, so it cannot drift from the build. Regenerate with
`node scripts/module-graph.mjs`; `--check` fails if it is stale.
...
graph LR
  ...
  secure_store["secure-store"] --> settings["settings"]
  ...
  designsystem["designsystem"] --> payments_api["payments-api"]
  ai["ai"] --> result["result"]
  ai["ai"] --> common["common"]
  ...
  provider_stripe["provider:stripe"] --> common["common"]
  provider_upi_intent["provider:upi-intent"] --> payments_api["payments-api"]
  provider_upi_intent["provider:upi-intent"] --> common["common"]
  provider_cashfree["provider:cashfree"] --> payments_api["payments-api"]
  ...
  provider_razorpay["provider:razorpay"] --> common["common"]
  provider_square["provider:square"] --> payments_api["payments-api"]
  provider_square["provider:square"] --> common["common"]
  provider_hosted_webview["provider:hosted-webview"] --> payments_api["payments-api"]
  provider_hosted_webview["provider:hosted-webview"] --> payments_api["payments-api"]
  provider_flutterwave["provider:flutterwave"] --> payments_api["payments-api"]
  provider_flutterwave["provider:flutterwave"] --> common["common"]
  provider_flutterwave["provider:flutterwave"] --> provider_hosted_webview["provider:hosted-webview"]
  provider_paystack["provider:paystack"] --> payments_api["payments-api"]
  provider_paystack["provider:paystack"] --> common["common"]
  provider_paystack["provider:paystack"] --> provider_hosted_webview["provider:hosted-webview"]
  provider_paytm["provider:paytm"] --> payments_api["payments-api"]
  provider_paytm["provider:paytm"] --> common["common"]
  provider_paytm["provider:paytm"] --> provider_hosted_webview["provider:hosted-webview"]
  provider_stripe_connect["provider:stripe-connect"] --> payments_api["payments-api"]
  provider_stripe_connect["provider:stripe-connect"] --> common["common"]
  provider_stripe_connect["provider:stripe-connect"] --> provider_hosted_webview["provider:hosted-webview"]
  provider_cash["provider:cash"] --> payments_api["payments-api"]
  provider_nmi["provider:nmi"] --> payments_api["payments-api"]
  provider_peach["provider:peach"] --> payments_api["payments-api"]
  provider_mobile_money["provider:mobile-money"] --> payments_api["payments-api"]
  provider_mobile_money["provider:mobile-money"] --> network["network"]
  provider_mpesa["provider:mpesa"] --> payments_api["payments-api"]
  provider_mpesa["provider:mpesa"] --> network["network"]
  provider_wallet["provider:wallet"] --> payments_api["payments-api"]
  provider_xendit["provider:xendit"] --> payments_api["payments-api"]
  provider_xendit["provider:xendit"] --> network["network"]
```

_43 modules, 60 internal dependencies._
<!-- module-graph:end -->
...
graph TD
    ...
    PROVIDERS["provider:* leaves<br/>stripe · razorpay · cashfree · square · …"]
...
| `ModelManager` | `interface { fun models(): List<ModelInfo>; fun observe(id): Flow<ModelInfo>; suspend fun download(id, licenseAcknowledged = false); suspend fun delete(id) }` | On-demand model download/residency status. `download` itself declines (state `FAILED`) a gated model without `licenseAcknowledged = true`, or any model over a metered connection — on Android via `MediaPipeModelManager` |
...
├── provider/                # gateway adapter modules (stripe, razorpay, cashfree, square, …)
...
- [x] `payments-api` and `provider:*` gateway leaves, one contract, sandbox-honest `GatewayStatus`
